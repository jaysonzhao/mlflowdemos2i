...
def prediction():
    ...
    user_input_ids = tokenizer.encode(question + tokenizer.eos_token, return_tensors='pt')


    # generated a response while limiting the total chat history to 1000 tokens, 
    answer_ids = model.generate(user_input_ids, max_length=1000, pad_token_id=tokenizer.eos_token_id)

    # pretty print tokens from bot
    mes = format(tokenizer.decode(answer_ids[:, user_input_ids.shape[-1]:][0], skip_special_tokens=True))
